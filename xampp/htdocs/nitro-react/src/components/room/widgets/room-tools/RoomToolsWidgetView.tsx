...
export const RoomToolsWidgetView: FC<{}> = props =>
{
    ...
    const onChangeRoomHistory = (roomId: number, roomName: string) =>
    {
        ...
        const newData = !newStorage ? [ { roomId, roomName } ] : [ ...newStorage, { roomId, roomName } ];
    ...
    return (
        <Flex className="nitro-room-tools-container holo-classic-room-tools" gap={ 1 }>
            <div className="btn-toggle toggle-roomtool d-flex align-items-center justify-content-center" onClick={ () => setShow(!show) }>
                <div className={ 'toggle-icon ' + (!show ? 'right' : 'left') } />
            </div>

            { show &&
                <>
                    <Column gap={ 1 } className="nitro-room-tools">
                        <Flex pointer alignItems="center" className="holo-room-tool-row" onClick={ () => handleToolClick('settings') }>
                            <Flex center className="holo-room-tool-icon"><Base className="icon icon-cog" /></Flex>
                            <Text className="holo-room-tool-label">{ LocalizeText('room.settings.button.text') }</Text>
                        </Flex>

                        <Flex pointer alignItems="center" className="holo-room-tool-row" onClick={ () => handleToolClick('zoom') }>
                            <Flex center className="holo-room-tool-icon">
                                <Base className={ classNames('icon', (!isZoomedIn && 'icon-zoom-less'), (isZoomedIn && 'icon-zoom-more')) } />
                            </Flex>
                        ...
                        <Flex pointer alignItems="center" className="holo-room-tool-row" onClick={ () => handleToolClick('chat_history') }>
                            ...
                            <Text className="holo-room-tool-label">{ LocalizeText('room.chathistory.button.text') }</Text>
                        </Flex>

                        { navigatorData.canRate &&
                            <Flex pointer alignItems="center" className="holo-room-tool-row" onClick={ () => handleToolClick('like_room') }>
                                <Flex center className="holo-room-tool-icon"><Base className="icon icon-like-room" /></Flex>
                                <Text className="holo-room-tool-label">{ LocalizeText('room.like.button.text') }</Text>
                            </Flex> }

                        <Flex pointer alignItems="center" className="holo-room-tool-row" onClick={ () => handleToolClick('toggle_room_link') }>
                            <Flex center className="holo-room-tool-icon"><Base className="icon icon-room-link" /></Flex>
                            <Text className="holo-room-tool-label">{ LocalizeText('navigator.embed.caption') }</Text>
                        </Flex>

                        <Flex pointer alignItems="center" className="holo-room-tool-row" onClick={ () => handleToolClick('hiddenbubbles') }>
                            <Flex center className="holo-room-tool-icon">
                                <Base className={ `icon ${ areBubblesMuted ? 'icon-chat-disablebubble' : 'icon-chat-enablebubble' }` } />
                            </Flex>
                            <Text className="holo-room-tool-label">{ areBubblesMuted ? LocalizeText('room.unmute.button.text') : LocalizeText('room.mute.button.text') }</Text>
                        </Flex>

                        <Flex justifyContent="center" alignItems="center" className="holo-room-history-controls">
                            ...
                            <Base pointer={ roomHistory?.length > 0 } title={ LocalizeText('room.history.button.tooltip') } className={ `icon ${ roomHistory?.length === 0 ? 'icon-room-history-disabled' : 'icon-room-history-enabled' }` } onClick={ () => roomHistory?.length === 0 ? null : handleToolClick('room_history') } />
                    ...
                    <Flex className="nitro-room-tools-history" style={ { bottom: !navigatorData.canRate ? '180px' : '210px' } }>
                        <TransitionAnimation type={ TransitionAnimationTypes.SLIDE_LEFT } inProp={ isOpenHistory }>
                            <Column center>
                                <Column className="px-3 py-2 nitro-room-history">
                                    <Column gap={ 1 }>
                    ...
                    <Column justifyContent="center">
                        <TransitionAnimation type={ TransitionAnimationTypes.SLIDE_LEFT } inProp={ isOpen } timeout={ 300 }>
                            <Column center>
                                <Column className="px-3 py-2 nitro-room-tools-info" overflow="hidden">
                                    <Column gap={ 1 }>
                                    ...
                                    { roomTags && roomTags.length > 0 &&
                                        <Flex gap={ 2 }>
                                            { roomTags.map((tag, index) =>
                                                <Text key={ index } small pointer truncate variant="white" className="holo-room-tag p-1" onClick={ () => handleToolClick('navigator_search_tag', tag) }>#{ tag }</Text>) }
                                        </Flex> }
                                </Column>
                            </Column>
                        </TransitionAnimation>
