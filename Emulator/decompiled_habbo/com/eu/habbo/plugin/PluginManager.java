...
public class PluginManager {
    ...
    public void updatePluginByName(String name) {
        ...
        for (HabboPlugin p : this.plugins) {
            if (!p.configuration.name.equalsIgnoreCase(name)) continue;
            pluginReload = p;
        }
        if (!loc.exists() && loc.mkdirs()) {
            LOGGER.info("Created plugins directory!");
        ...
        for (File file2 : Objects.requireNonNull(loc.listFiles(file -> file.getPath().toLowerCase().endsWith(".jar")))) {
            try {
                ...
                try {
                    ...
                    plugin.classLoader = urlClassLoader;
                    plugin.stream = stream;
                    if (!plugin.configuration.name.equalsIgnoreCase(name) || pluginReload == null || !this.plugins.contains(pluginReload) || !this.plugins.remove(pluginReload)) continue;
                    this.plugins.add(plugin);
                    plugin.onEnable();
