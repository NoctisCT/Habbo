...
public class WiredHighscoreManager {
    ...
    public List<WiredHighscoreRow> getHighscoreRowsForItem(int itemId, WiredHighscoreClearType clearType, WiredHighscoreScoreType scoreType) {
        ...
        if (scoreType == WiredHighscoreScoreType.LONGESTTIME) {
            return highscores.collect(Collectors.groupingBy(h -> h.getUsers().hashCode())).entrySet().stream().map(e -> ((List)e.getValue()).stream().max(Comparator.comparingInt(WiredHighscoreRow::getValue)).orElse(null)).filter(Objects::nonNull).sorted(Comparator.comparingInt(WiredHighscoreRow::getValue).reversed()).collect(Collectors.toList());
        }
        return null;
    }
