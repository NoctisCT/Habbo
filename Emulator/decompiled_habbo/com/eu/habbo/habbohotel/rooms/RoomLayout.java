...
public class RoomLayout {
    ...
    public final Deque<RoomTile> findPath(RoomTile oldTile, RoomTile newTile, RoomTile goalLocation, RoomUnit roomUnit, boolean isWalktroughRetry) {
        ...
        LinkedList<RoomTile> openList = new LinkedList<RoomTile>();
        LinkedList<RoomTile> closedList = new LinkedList<RoomTile>();
        openList.add(oldTile.copy());
        RoomTile doorTile = this.room.getLayout().getDoorTile();
        long startMillis = System.currentTimeMillis();
        while (!openList.isEmpty()) {
            if (System.currentTimeMillis() - startMillis > (long)Emulator.getConfig().getInt("pathfinder.execution_time.milli", 25) && Emulator.getConfig().getBoolean("pathfinder.max_execution_time.enabled", false)) {
                return null;
            }
            ...
            for (RoomTile currentAdj : adjacentNodes) {
                ...
                if (currentAdj.hasUnits() && doorTile.distance(currentAdj) > 2.0 && (!isWalktroughRetry || !this.room.isAllowWalkthrough() || currentAdj.equals(goalLocation))) {
                    closedList.add(currentAdj);
                    openList.remove(currentAdj);
                    continue;
                }
                ...
                if (currentAdj.getgCosts() <= currentAdj.calculategCosts(current)) continue;
                currentAdj.setPrevious(current);
                currentAdj.setgCosts(current);
            }
        }
    ...
    public Deque<RoomTile> calcPath(RoomTile start, RoomTile goal) {
        LinkedList<RoomTile> path = new LinkedList<RoomTile>();
        if (start == null) {
            return path;
        }
        RoomTile curr = goal;
    ...
    private List<RoomTile> getAdjacent(List<RoomTile> openList, RoomTile node, RoomTile nextTile, RoomUnit unit) {
        ...
        short x = node.x;
        short y = node.y;
        LinkedList<RoomTile> adj = new LinkedList<RoomTile>();
        if (x > 0 && this.canWalkOn(temp = this.findTile(openList, (short)(x - 1), y), unit) && (temp.state != RoomTileState.SIT || nextTile.getStackHeight() - node.getStackHeight() <= 2.0)) {
            temp.isDiagonally(false);
            if (!adj.contains(temp)) {
                adj.add(temp);
            }
        }
        if (x < this.mapSizeX && this.canWalkOn(temp = this.findTile(openList, (short)(x + 1), y), unit) && (temp.state != RoomTileState.SIT || nextTile.getStackHeight() - node.getStackHeight() <= 2.0)) {
            temp.isDiagonally(false);
            if (!adj.contains(temp)) {
                adj.add(temp);
            }
        }
        if (y > 0 && this.canWalkOn(temp = this.findTile(openList, x, (short)(y - 1)), unit) && (temp.state != RoomTileState.SIT || nextTile.getStackHeight() - node.getStackHeight() <= 2.0)) {
            temp.isDiagonally(false);
            if (!adj.contains(temp)) {
                adj.add(temp);
            }
        }
        if (y < this.mapSizeY && this.canWalkOn(temp = this.findTile(openList, x, (short)(y + 1)), unit) && (temp.state != RoomTileState.SIT || nextTile.getStackHeight() - node.getStackHeight() <= 2.0)) {
            temp.isDiagonally(false);
            if (!adj.contains(temp)) {
                adj.add(temp);
            }
        ...
        if (this.CANMOVEDIAGONALY) {
            ...
            if (x < this.mapSizeX && y < this.mapSizeY) {
                offX = this.findTile(openList, (short)(x + 1), y);
                offY = this.findTile(openList, x, (short)(y + 1));
                if (offX != null && offY != null && (this.canMoveDiagonally || offX.isWalkable() || offY.isWalkable()) && this.canWalkOn(temp2 = this.findTile(openList, (short)(x + 1), (short)(y + 1)), unit) && (temp2.state != RoomTileState.SIT || nextTile.getStackHeight() - node.getStackHeight() <= 2.0)) {
                    temp2.isDiagonally(true);
                    if (!adj.contains(temp2)) {
                        adj.add(temp2);
                    }
                }
            }
    ...
    public boolean fitsOnMap(RoomTile tile, int width, int length, int rotation) {
        block4: {
            block5: {
                if (tile == null) break block4;
                if (rotation != 0 && rotation != 4) break block5;
                for (short i = tile.x; i <= tile.x + (width - 1); i = (short)(i + 1)) {
                    for (short j = tile.y; j <= tile.y + (length - 1); j = (short)(j + 1)) {
                        ...
                        if (t != null && t.state != RoomTileState.INVALID) continue;
                        return false;
                    }
                }
                break block4;
            }
            if (rotation != 2 && rotation != 6) break block4;
    ...
    public THashSet<RoomTile> getTilesAt(RoomTile tile, int width, int length, int rotation) {
        ...
        block4: {
            block5: {
                pointList = new THashSet<RoomTile>(width * length, 0.1f);
                if (tile == null) break block4;
                if (rotation != 0 && rotation != 4) break block5;
                for (short i = tile.x; i <= tile.x + (width - 1); i = (short)(i + 1)) {
                    for (short j = tile.y; j <= tile.y + (length - 1); j = (short)(j + 1)) {
