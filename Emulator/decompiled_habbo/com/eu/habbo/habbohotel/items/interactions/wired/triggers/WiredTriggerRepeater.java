...
WiredTriggerReset {
    ...
    public void serializeWiredData(ServerMessage message, Room room) {
        ...
        if (!this.isTriggeredByRoomUnit()) {
            ...
            room.getRoomSpecialTypes().getEffects(this.getX(), this.getY()).forEach(new TObjectProcedure<InteractionWiredEffect>(){
                final /* synthetic */ WiredTriggerRepeater this$0;
                {
                    this.this$0 = this$0;
                }
    ...
    public void cycle(Room room) {
        ...
        String Key2 = Double.toString(this.getX()) + Double.toString(this.getY());
        room.repeatersLastTick.putIfAbsent(Key2, currentMillis);
        if (this.counter >= this.repeatTime && room.repeatersLastTick.get(Key2) < currentMillis - 450L) {
            this.counter = 0;
            if (this.getRoomId() != 0 && room.isLoaded()) {
                room.repeatersLastTick.put(Key2, currentMillis);
