...
extends InteractionWiredExtra {
    public List<Integer> seenList = new ArrayList<Integer>();
    ...
    public InteractionWiredEffect getUnseenEffect(List<InteractionWiredEffect> effects) {
        ArrayList<InteractionWiredEffect> unseenEffects = new ArrayList<InteractionWiredEffect>();
        for (InteractionWiredEffect effect : effects) {
            if (this.seenList.contains(effect.getId())) continue;
            unseenEffects.add(effect);
        }
        InteractionWiredEffect effect = null;
        if (!unseenEffects.isEmpty()) {
            effect = (InteractionWiredEffect)unseenEffects.get(0);
        } else {
            this.seenList.clear();
            if (!effects.isEmpty()) {
