...
implements InteractionWiredMatchFurniSettings {
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
        ...
        for (WiredMatchFurniSetting setting : this.settings) {
            ...
            if (this.state && this.checkForWiredResetPermission && item.allowWiredResetState() && !setting.state.equals(" ") && !item.getExtradata().equals(setting.state)) {
            ...
            if (this.direction && !this.position) {
                if (item.getRotation() == setting.rotation || room.furnitureFitsAt(oldLocation, item, setting.rotation, false) != FurnitureMovementError.NONE) continue;
            ...
            int n = newRotation = this.direction ? setting.rotation : item.getRotation();
            if (newLocation == null || newLocation.state == RoomTileState.INVALID || newLocation == oldLocation && newRotation == item.getRotation() || room.furnitureFitsAt(newLocation, item, newRotation, true) != FurnitureMovementError.NONE || room.moveFurniTo(item, newLocation, newRotation, null, !slideAnimation) != FurnitureMovementError.NONE || !slideAnimation) continue;
