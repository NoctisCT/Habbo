...
class DatabasePool {
    ...
    public boolean getStoragePooling(ConfigurationManager config) {
        try {
            ...
            this.log.info("INITIALIZING DATABASE SERVER: " + config.getValue(DB_HOSTNAME_KEY));
            this.log.info("ON PORT: " + config.getValue(DB_PORT_KEY));
            this.log.info("HABBO DATABASE: " + config.getValue(DB_NAME_KEY));
            this.database = new HikariDataSource(databaseConfiguration);
        }
        catch (Exception e) {
            this.log.error("Error initializing database connection pool: {}", (Object)e.getMessage());
            return false;
        }
        return true;
    }
