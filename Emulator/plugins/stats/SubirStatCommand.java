...
public class SubirStatCommand extends Command {
    public SubirStatCommand() {
        super("cmd_subir", new String[] { "subir", "subirstat" });
    }
    ...
    public boolean handle(GameClient client, String[] params) throws Exception {
        ...
        if (params.length < 3) {
            habbo.whisper("Uso correcto: :subir [stat] [valor]");
            return true;
        }

        String stat = params[1].toLowerCase();
        int valor;

        try {
            valor = Integer.parseInt(params[2]);
            if (valor < 0) {
        ...
        UserCombatStats stats = CombatManager.getStats(habbo.getHabboInfo().getId());
        ...
        if (!statValido) {
            habbo.whisper(
                    "Atributo inválido. Elige: vitalidad, max_vitalidad, reiryoku, max_reiryoku, fuerza, defensa, velocidad, voluntad o reiatsu.");
            return true;
        }
        ...
        habbo.whisper("¡Atributo " + stat + " establecido en " + valor + "!");
        return true;
    }
}